...
const BackendVM = require('./backend-vm')

// VM factory
// =

class VMFactory extends BackendVM {
  constructor ({maxVMs}) {
  ...
  async provisionVM ({code, title}) {
    ...
    // initiate vm
    const backendVM = new BackendVM(code)
    const dir = path.join(this.dir, backendVM.id)
    await backendVM.deploy({dir, title})
    this.numVMs++
    this.VMs[backendVM.id] = backendVM
    backendVM.on('close', () => {
      this.numVMs--
      delete this.VMs[backendVM.id]
    })

    // mount to the server
    this.rpcServer.mount('/' + backendVM.id, this)

    return {
      id: backendVM.id,
      callLogUrl: backendVM.callLog.url,
      filesArchiveUrl: backendVM.filesArchive.url
    }
  }
...
const FACTORY_SCRIPT = `
  ...
  async function assertPermission (record) {
    ...
    const callerId = backend.caller.id
